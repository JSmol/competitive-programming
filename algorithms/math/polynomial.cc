namespace polynomial {
  ...
  void mult(vector<T> &a, const vector<T> &b) {
    if (min(sz(a), sz(b)) < magic) { slow(a, b); return; }
    init();
  ...
  template<int m>
	struct modular {
		...
		modular operator / (const modular &t) const { return (*this * t.inv()) % m; }
  ...
  // interpolates minimum polynomial from (xi, yi) pairs (call this)
  template<typename T>
...
// example with piecewise polynomial integration...
...
poly<b> p(vector<b>({1,2,3,4,5}));
auto xs = vector<b>({5,4,3,2,1});
auto ys = p.eval(xs);
