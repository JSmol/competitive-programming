...
namespace geometry {
  ...
  struct Point {
    double x, y;
    bool operator==(const Point &p) const { return dEqual(x, p.x) and dEqual(y, p.y); }
    bool operator<(const Point &p) const { return y < p.y or (y == p.y and x < p.x); }
  };

  ostream& operator<<(ostream& o, const Point& x) { o << "(" << x.x << ", " << x.y << ")"; return o; }
  ...
  int convex_hull(Point P[], int n, Point hull[]){
    ...
    for (int i = 0; i < n; i++) {
      while (sz(L) > 1 and cross(P[i]-L.back(), L[sz(L)-2]-P[i]) < -EPS) L.pop_back();
      while (sz(U) > 1 and cross(P[i]-U.back(), U[sz(U)-2]-P[i]) >  EPS) U.pop_back();
      L.push_back(P[i]); U.push_back(P[i]);
    }
    copy(L.begin(), L.end(), hull); copy(U.rbegin()+1, U.rend()-1, hull+sz(L));
    return sz(L) + sz(U) - 2;
  }
  ...
  bool point_in_poly(Point P[], int n, Point p) {
    int i, j, c = 0;
    for (i = 0; i < n; i++) 
      if (P[i] == p or	ccw(P[i], P[(i+1)%n], p) == CNEITHER) return BOUNDARY;
    for (i = 0, j = n-1; i < n; j = i++) 
      if (((P[i].y <= p.y and p.y < P[j].y) or (P[j].y <= p.y and p.y < P[i].y)) 
      and (p.x < (P[j].x - P[i].x) * (p.y - P[i].y) / (P[j].y - P[i].y) + P[i].x))
        c = !c;
    return c;
  }
  ...
  int intersect_line(Point a, Point b, Point c, Point d, Point &p, bool segment) {
    ...
    if (!dEqual(denom, 0)) {
      ...
      if (!segment or (0-EPS <= r and r <= 1+EPS and 0-EPS <= s and s <= 1+EPS)) {
        p = a + r*(b-a); return 1;
      } else return 0;
    }
    ...
    if (a.x == b.x) {
      ...
      return (b.y < c.y or d.y < a.y) ? 0 : -1;
    } else if (b.x == c.x) { p = b; return 1; }
    else if (a.x == d.x) { p = a; return 1; }
    else if (b.x < c.x or d.x < a.x) return 0;
    return -1;
  }
  ...
  int intersect_circle_circle(Circle c1, Circle c2, Point& ans1, Point& ans2) {
    if (c1.c == c2.c and dEqual(c1.r, c2.r)) return 3;
    double d = len(c1.c-c2.c);
    if (d > c1.r + c2.r + EPS or d < fabs(c1.r-c2.r) - EPS) return 0;
  ...
  Point M;
  bool left_half(Point p) { return p.x < M.x or (p.x == M.x && p.y > M.y); }
  double cp(Point P[], int n, vector<Point>& X, int l, int h) {
    if (h - l == 2) return len(P[l]-P[l+1]);
    if (h - l == 3) return min(len(P[l]-P[l+1]), min(len(P[l]-P[l+2]), len(P[l+1]-P[l+2])));
    M = X[(h+l)/2]; int m = stable_partition(P+l, P+h, left_half)-P;
    double d = min(cp(P, n, X, l, m), cp(P, n, X, m, h));
    M.x += d, M.y = INF; int t = stable_partition(P+m, P+h, left_half)-P;
    for (int i = l, j = m; i < m and j < t; i++) { if (P[m].x - P[i].x >= d) continue;
      while (j < t and P[i].y - P[j].y >= d) j++;
      for (int k = j; k < t and P[k].y-P[i].y < d; k++)
  ...
  double closest_pair(Point P[], int n) { // Call this from your program
    ...
    sort(all(X)); for (int i = 0; i < n; i++) X[i] = -1*inv(X[i]);
    return cp(P, n, X, 0, n);
  }
...
vector<Point> P = {
  ...
  // Orientation test  (When pts are colinear: ccw: a-b-c  cw: c-a-b  neither: a-c-b)
int check(vector<int> A, string s) {
  for (int i = 1; i+1 < 9; i++) {
    char D = '#';
    auto x = ccw(P[A[i-1]], P[A[i]], P[A[i+1]]);
    if (colinear(P[A[i-1]], P[A[i]], P[A[i+1]])) {
      if (x == CCW) {
        D = 'S';
      } else {
        D = 'A';
  ...
  for (int i = 0; i+1 < 9; i++) {
    S.insert(A[i+1]);
    for (int j = 1; j <= 9; j++) 
      if (!S.count(j) and colinear(P[A[i]], P[j], P[A[i+1]]) and ccw(P[A[i]], P[j], P[A[i+1]]) == CCW)
        return 0;
  }
