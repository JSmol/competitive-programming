...
piecewise integrate(piecewise& p, int bound) {
  ...
  for (int i = 1; i < sz(f); i++) {
    if (r[i] <= bound) {
      ...
      // debug(bound);
      // debug(f[i].a);
      // debug(poly<b>(f[i].eval(min(r[i], bound))).a);
...
piecewise f(int v) {
  V[v] = 1;
  piecewise p;
  p.f.push_back({0}); p.r.push_back({0}); 
  p.f.push_back({b(1)/b(B[v])}); p.r.push_back({B[v]});
  p.f.push_back({0}); p.r.push_back({1 << 30});
  ...
  // is 0 when x < 0 and x > B[i]
  // if (v == root) { for (auto& x : p.f) debug(x.a); cerr << "after\n"; }
  p = integrate(p, B[v]);
  // if (v == root) { for (auto& x : p.f) debug(x.a); cerr << endl; }
