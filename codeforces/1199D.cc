...
int main() {
  ...
  for (int i = 0; i < n; i++) l[i] = { 0, A[i] };
  segment<int> s(q, -(1 << 30), [] (int a, int b) { return max(a, b); });
  for (int i = 0; i < q; i++) {
    int t, x, y; cin >> t;
    if (t == 1) cin >> x >> y, l[x-1] = {i, y};
    else cin >> x, s.update(i, x);
  }

  for (int i = 0; i < n; i++) {
    cout << max(l[i].snd, s.query(l[i].fst, q)) << ' ';
  }
}
